...
- 点击卡片标题或“详情”打开右侧面板，集中编辑实体、添加或修改属性、管理关系。卡片仅保留属性摘要和“查询”入口；连接点在选中卡片时显示，连接过程中显示全部目标连接点。
- 键盘操作：聚焦卡片标题后用方向键移动，**Shift** 加大步长，**Enter** 打开详情；连接点也支持 **Enter**。聚焦画布后可用方向键平移、**+ / −** 缩放、**0** 适配。列表模式支持不使用画布手势完成定义编辑。
...
点击实体卡片的 **查询 →**，选择数据源，即可 **关联已有查询**、**创建查询** 或 **预览** 已发布的可执行查询。侧栏分别显示已发布状态和草稿关联。源草稿尚未映射此概念时，先点击 **配置映射**。

关联已有查询只修改该查询的概念引用，保留原生查询及参数值，并使用当前修订保存。并发修改会拒绝保存；重新加载并审阅后再操作。从概念创建查询会自动携带数据源和概念，**返回业务概念** 可以回到原实体与数据源。

两种查询编辑器均提供 **业务概念 → 选择概念**，按名称检索已映射实体、有效属性和关系，点击已选概念可查看定义。选项使用源草稿固定的本体版本；失效的已有引用会保留供修正。独立查询仍可不关联本体。

图上的 **仅有定义**、**已映射 · 暂无查询**、**查询可用**、**查询不可用** 反映已发布内容。草稿关联不会提前对 Agent 生效；审阅并发布整份数据源草稿后才生效。纯概念关联修改保留试跑证据和执行版本，执行仍检查验证状态与当前授权。

已发布查询详情展示关联概念定义。MCP `get_semantic_entry` 的 `related_entries` 同时提供该源可见的概念名称与引用，支持双向发现。这些关联提供业务上下文，不生成查询、不自动绑定参数和属性、不转换结果，也不增加访问权限。
...
- 在 **查询工具** 显式选择模板关联的业务概念。这些引用随查询结果返回；关系及计算属性的模板引用同时提供发现指导。
...
实体查询面板默认展示已有该实体发布映射的数据源；只有一个时直接选中。通过“连接其他数据源”查看或配置其他来源。草稿映射仍需审阅发布后生效。
