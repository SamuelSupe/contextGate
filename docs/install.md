...
[简体中文](install.zh-CN.md) · [Release](https://github.com/SamuelSupe/contextGate/releases/tag/v0.8.0)

**ContextGate 0.8.0** uses the `contextgate` executable and retains `mcpdbhub` as an alias.

## Upgrade to 0.8.0

From **0.6.x or 0.7.x**, back up the PostgreSQL metadata database and matching master key, stop the service, then replace the package while preserving both and the deployment configuration. No new metadata migration or forced token rotation is introduced. See the [0.8.0 upgrade notes](releases/0.8.0.md#compatibility-and-upgrade).
