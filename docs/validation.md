...
日期：2026-09-11。后端与数据库测试运行于 OrbStack Linux arm64；管理界面通过本机 Chrome 实际操作验证，没有引入浏览器自动化框架。
...
| 数据库矩阵 | 18 产品、20 个独立版本组合通过；[逐项版本记录](verification/)与[能力限制](support-matrix.md) |
...
当前本地服务运行已验证镜像中的二进制。仅清理本轮临时 OAuth 客户端、数据源和数据库容器；保留原有配置、撤销状态及审计记录。密码恢复使用隔离配置卷验收，没有更改本地服务的管理员密码。[中文恢复说明](../README.md)与[英文恢复说明](../README.en.md)已更新。
