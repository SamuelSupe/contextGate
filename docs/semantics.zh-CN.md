...
每个数据源拥有独立的业务术语、对象、字段、关系、指标和原生查询模板。继续采用 **Agent → Data Source** 授权。语义只描述业务，不承担权限控制，也不能覆盖 Agent 的系统指令。界面默认英文，可在设置中切换简体中文；名称、别名和说明支持中文及其他语言。

当前 JSON 导出采用**格式 v2**，可包含[共享本体映射](ontologies.zh-CN.md)。继续支持 v1 导入；仅升级格式不会使已有试跑证据失效。

已有查询可从 **首页 → 新建查询** 进入[连续发布流程](query-publishing.zh-CN.md)。查询目录、数据源语义页和本体入口使用同一编辑器，“查询 / 参数 / 业务说明”共享同一草稿，参数示例与契约放在一起。下面的数据源工作流继续用于完整目录维护。
...
3. 在 **Query templates** 使用 SQL/CQL/Cypher/InfluxDB 文本编辑器，或在高级 JSON 编辑器维护完整请求；HTTP API 可选择已配置操作自动填入契约。设置参数类型、允许的绑定位置、约束、示例参数和结果说明。**Find parameter positions** 只列出现有参数槽，不创建占位符或插值。指标编辑器可以关联一个模板。
4. 保存草稿后运行 **Validate draft**；新增或修改的启用模板必须点击 **Trial**，使用保存的示例参数和已配置的[回归检查](operations.zh-CN.md#模板回归检查)，对真实数据源完成只读试跑。
5. 打开 **Review and publish**，补齐试跑或映射检查、核对变更并确认，原子替换 Agent 可见的完整快照。**Discard draft** 用已发布快照恢复草稿。旧修订保存或发布返回 HTTP 409，不覆盖其他页面的更新。
...
HTTP API 使用**管理员维护的 API 契约版本**，不是自动检测的服务端版本。上游契约变化时需要更新该版本；API 配置或凭证变化会使证据过期，无法自动发现上游变化。详见 [HTTP API 契约](http-api.zh-CN.md)。

失效后需要 **重新 Trial，再 Publish**。单独试跑不会恢复旧发布模板；重新发布会生成新的执行版本。连接检查不会发送试探性写入，也不会把账号权限“未验证”误报成“已只读验证”。

## 参数契约

模板包含 `tool`、`query_json`、`parameters`、`example_json`、`enabled`，以及可选 `result_description`、`test_cases`。配置中的原生查询和 JSON 参数值使用字符串保存，避免浏览器通过 JavaScript Number 损失大整数或 Decimal 精度。Agent 调用时提交真实 JSON 参数值，Go 使用无损数字解码。
...
| HTTP API | 固定操作中已有的标量 `/named_params/name` 槽；操作 ID 和请求结构保持固定 |
...
`list_data_sources` 包含 `query_access_mode`、`semantics_available`、`semantic_version`、`available_tools`，不一次返回目录。

```json
{"name":"search_semantics","arguments":{"source_id":"src_example","keyword":"金额","kind":"metric","limit":25}}
```

按名称、别名、说明关键词匹配。类型包括 `overview`、`term`、`object`、`field`、`relationship`、`metric`、`template`、`entity_type`、`property`、`relation_type`。本体条目使用结构发现返回的独立概念 ID，只展示当前源映射的可见子集。每页最多 100 条摘要、64 KiB 摘要数据。游标有效期 5 分钟，绑定身份、数据源、关键词、类型、分页大小、源发布版本和采用的本体版本；发布版本改变后重新开始检索。
...
Agent 只能提交模板标识、精确执行版本、参数，以及可选游标和更严格的上限。结果保留原生结构，并追加 `semantic_version`、`template_id`、`template_version`。采用本体时，`ontology_context` 记录请求开始时的本体版本及模板显式关联概念，不转换值或推断关系。执行前和返回前检查授权及模板有效性。原生分页游标额外绑定模板 ID、执行版本、查询、参数、身份；原生工具不能消费模板游标。SQL 继续由显式查询分页。
...
导出 JSON 包含 `format_version: 2`、`overview`、`entries` 和可选 `ontology`；不含连接配置、凭证、验证证据或执行批准。导入使用明确的替换流程，只影响草稿。删除数据源会在事务内删除草稿、发布内容、发布历史和验证证据。[发布历史](operations.zh-CN.md#变更追溯与版本恢复)支持比较及恢复为草稿；映射检查和采用版本的接口见[本体指南](ontologies.zh-CN.md)。
...
InfluxDB 2 使用数据库的[结构发现函数](https://docs.influxdata.com/influxdb/v2/query-data/flux/explore-schema/)，范围为最近 30 天；字段类型不进行猜测。HTTP API 只导入管理员声明的操作和响应字段，字段仍标记未验证。上述结构只用于生成骨架，均不读取业务样本来推断业务含义，不连接额外模型。

## 审计和示例

模板执行和管理员试跑沿用脱敏审计，分别保留实际管理员或个人配置 MCP 身份，以及执行所用的查询 Agent。已发布执行记录操作 `execute_query_template`、模板 ID、执行版本；OTLP 属性为 `mcpdbhub.audit.template_id`、`mcpdbhub.audit.template_version`。审计不保存目录全文、原生查询文本、参数、结果或连接秘密。

[示例目录](../examples/semantics/) 覆盖 PostgreSQL SQL、位置参数 SQL、ClickHouse、MongoDB、Redis、Search、Cypher、CQL 及三个 InfluxDB 版本。另外提供 [HTTP API 目录示例](../examples/http-api/semantics.json)。请按实际结构修改固定查询和业务定义，导入后试跑并发布。实际数据库验证结果见[验证记录](validation.zh-CN.md)。
