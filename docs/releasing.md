...
The examples below use **0.8.0**; update the version for subsequent releases. New artifacts and launchers use `contextgate`; an additional `mcpdbhub` launcher preserves existing scripts.
...
2. Build the UI with `make ui`. Run `go test ./...`, `go test -race ./...`, `npm --prefix web test`, and `node --test web/test/query.test.mjs` in the appropriate environments.
