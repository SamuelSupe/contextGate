...
2026-09-11，OrbStack Linux arm64 独立实例验收。**18 个产品、20 个产品/版本组合**；InfluxDB 1.x、2.x、3 Core 分开计算。每个产品均有独立连接、结构发现、参数化查询、类型、空结果、错误、危险操作拒绝、目标数据不变、行数限制及取消检查。原始通过记录在 [verification](verification/)，复现方式见 [validation](validation.md)。
