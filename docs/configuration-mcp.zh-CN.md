...
预览云数仓的连接字段、凭证与限制见[云数仓指南](cloud-warehouses.zh-CN.md)和[数据源/语义示例](../examples/cloud-warehouses/README.md)。BigQuery 服务账号 JSON 使用 `auth_mode: "service_account"`，存入加密的 password 字段；配置指引工具也提供各连接器的选项。目前尚未在真实云环境验证。
