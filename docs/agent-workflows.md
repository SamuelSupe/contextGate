...
Open **Evaluate** from a source's setup or query workspace. The default **Single answer check** captures one real client run for manual review. **Compare baseline and semantic guidance** is an optional two-run mode. Neither mode embeds a model or grades answers automatically.

1. Choose a dedicated Agent that has no unrelated calls during the evaluation. Record a question name, client/model/settings, business question and acceptance criteria. Use **Save question for reuse** to add it to **Saved questions**, or start an ad-hoc evaluation. Each mode provides a copyable prompt; acceptance criteria remain separate so the client does not receive the expected answer.
2. In **Single answer check**, start the capture, ask the selected Agent in a real client, wait for all tool calls to finish, then collect the completed calls.
3. For an optional comparison, choose **Compare baseline and semantic guidance** before starting. Capture **Baseline** using the existing workflow, then **With semantic guidance** in a separate conversation with the same model and data, using semantic discovery and verified templates when applicable.
...
Opening an evaluation shows its results first: one captured run for a single check, or baseline/guided runs side by side for a comparison. Question, expected answer, conditions and review controls are expandable. Manual review changes must be saved; a confirmation appears after saving. Repeat creates a new evaluation using the existing inputs.

History filters question name/text, Agent and creation date across the entire source history, with an inclusive end day in the browser's local time zone. Applied filters and pagination are retained while opening a record and returning to History within the same workspace. Summary counts cover every matching page and keep single checks separate from paired comparisons. Single-check correctness includes only completed, reviewed captures with query calls and unchanged configuration. Paired comparisons require both captures to meet those conditions. Unrated, empty and changed-configuration pairs are excluded. Different questions, models or underlying data can still affect outcomes; the summary does not establish a causal model-quality improvement.
