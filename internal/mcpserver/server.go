...
func Server(e *engine.Engine, p model.Principal) *mcp.Server {
	...
	for _, tool := range []struct {
		...
		{"query_http_api", "query_http_api", "Call an administrator-declared read-only HTTP API operation using its ID and scalar named_params. Discover operation contracts with list_objects and describe_object. URLs, methods, headers and arbitrary body fragments are not accepted. HTTP read-only behavior is administrator-declared.", []string{"operation", "named_params"}, []string{"source_id", "operation"}},
