...
func (s *Server) configurationGuide() any {
	return map[string]any{
		...
		"editing_rules": []string{
			...
			"SQL/CQL/Cypher/InfluxDB use native parameter slots; Redis binds fixed command argument elements; MongoDB/Search bind document value locations. HTTP API binds scalar /named_params/name values for a fixed operation; only declared request destinations are used. Never bind query text, commands, object names, keys or operators, and never interpolate strings.",
			"The tool descriptions and database support catalog cover the database query families and configured HTTP API operations. Trial uses the existing read-only engine. Template trial evidence excludes returned rows.",
			"HTTP API sources require an administrator-maintained API contract version, a fixed base URL, declared read-only GET/POST operations, scalar parameter contracts, and a connection-check operation. API response fields are declared metadata; upstream permissions and version are not inferred. Update the contract version when upstream changes, then trial and publish templates again.",
			"Preview cloud warehouses have no verified vendor environments. Snowflake: token authentication, options warehouse/role/schema/token_type (OAUTH or PROGRAMMATIC_ACCESS_TOKEN), positional params with ?. Databricks: token authentication, database=catalog, options warehouse_id/schema, named_params with :name. BigQuery: database=billing project, fixed host bigquery.googleapis.com, options location/schema/maximum_bytes_billed (default 1073741824), named_params with @name; service_account JSON goes in password, or use token authentication. These three require port 443, TLS verify, options read_only_confirmed=true and an administrator-maintained version; they accept only a restricted SELECT subset and do not verify account grants. Redshift uses database username/password, port 5439, PostgreSQL protocol, $1 parameters and per-query read-only transactions; IAM/Data API is not implemented. All four use query_sql and the existing semantic/ontology workflow.",
